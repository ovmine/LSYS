...
public class OrientationListener implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {  // 방향 센서 값이 바뀔때마다 호출됨
        ...
        if (deltaX > 0.8 && mAutoFocus) { //AUTOFOCUS (while it is not autofocusing)
            mAutoFocus = false;
            setCameraFocus(myAutoFocusCallback); //오토포커스 호출
            return;
        }
        if (deltaY > 0.8 && mAutoFocus) { //AUTOFOCUS (while it is not autofocusing)
            mAutoFocus = false;
            setCameraFocus(myAutoFocusCallback);
            return;
        }
        if (deltaZ > 0.8 && mAutoFocus) { //AUTOFOCUS (while it is not autofocusing) */
            mAutoFocus = false;
            setCameraFocus(myAutoFocusCallback);
            return;
        }
