...
public class LSYSUtility {
    ...
    public static Bitmap GetBitmapImage(final  Context context, final  int resourceId)
    {
        Bitmap temBitmap;
        Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), resourceId);

        if (bitmap.getHeight() > MainActivity.TEX_MAX_SIZE){
    ...
    // NORMAL 저장
    public static Thread Save(Context context, TextureView textureView, Object saveObject)
    {
        Thread svaeThread = new Thread(new ImageSaver(context,textureView, saveObject));
        svaeThread.start();
        return svaeThread;
    }

    // GIF 저장
    public static Thread Save(Context context, ArrayList<Bitmap> bitmapList, int goalCount, Object saveObject, Animation animation, int gifText)
    {
        Thread svaeThread = new Thread(new ImageSaver(context,bitmapList, goalCount, saveObject ,animation, gifText));
        svaeThread.start();
        return svaeThread;
    }

    // COLLAGE 저장
    public static Thread Save(Context context, TextureView textureView,Bitmap bitmap, Object saveObject)
    {
        Thread svaeThread = new Thread(new ImageSaver(context, textureView, bitmap, saveObject));
        svaeThread.start();
        return svaeThread;
    }
}
